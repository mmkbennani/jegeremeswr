    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            _setup: function () {
                ...
                this.language.edit = { title: this.language.edit.title || 'Modification d\'un Canal',
                                       button: this.language.edit.button || 'Sauvegarder'
                                     };
                this.language.delete = this.language.delete || {};
                this.language.delete = { title: this.language.delete.title || 'Suppression d\'un Canal',
                                         button: this.language.delete.button || 'Supprimer' };
                this.language.add = this.language.add || {};
                this.language.add = { title: this.language.add.title || 'Ajout d\'un nouveau Canal',
                                      button: this.language.add.button || 'Sauvegarder'
                                    };
				...
				this.language.errorinsert = this.language.errorinsert || {};
                this.language.errorinsert = { message: 'Le nom du canal est obligatoire',
											  label: 'Error !',
											  responseCode: 'Response code: ',
											  required: 'Field is required',
											  unique: 'Duplicated field'
											};
                var modal = '<div class="modal fade" id="' + modal_id + '" tabindex="-1" role="dialog" ata-keyboard="false" data-backdrop="static">' +
            ...
            _openEditModal: function () {
                ...
                for (var j in columnDefs) {
					...
					if(columnDefs[j].type == "checkbox"){
						if(selectedValue == false){
							$(selector).find(jquerySelector).removeAttr( "checked" )
						}else{
							$(selector).find(jquerySelector).attr( "checked", true )
						}
						$(selector).find(jquerySelector).trigger("change");
                    }else{
            ...
            _editRowData: function () {
                ...
                $(`form[name="altEditor-edit-form-${this.random_id}"] *`).filter(':input').each(function (i) {
					...
					if($(this)[0].outerHTML.indexOf("checkbox") >= 0){
						rowDataArray[$(this).attr('id')] = $(this)[0].checked;
					}else{
		...
		//Getting the textArea from the modal
            ...
            */
                    ...
                    if (columnDefs[j].type.indexOf("hidden") >= 0) {
                        ...
                        if (columnDefs[j].type.indexOf("readonly") >= 0) {
                            ...
                            data += "<input type='text' readonly  id='"
                                ...
                                + "' placeholder='";
							if(	buttonClass == "addRowBtn"){
								if(columnDefs[j].name.indexOf("date_disponnible") >= 0){
									...
									var currentDate =  year+"-"+month+"-"+day+"T"+hours+":"+minutes+":59";
									data += this._quoteattr(columnDefs[j].title)
									+ "' style='overflow:hidden'  class='form-control  form-control-sm' value='"+currentDate+"'>";
								}else if(columnDefs[j].name.indexOf("fk_country") >= 0){
									data += this._quoteattr(columnDefs[j].title)
									+ "' style='overflow:hidden'  class='form-control  form-control-sm' value='"+sessionStorage.val_sel_country+"'>";
								}else{
									data += this._quoteattr(columnDefs[j].title)
			...
			//Adding Text Area 
                        ...
                        {
							data += '<input type="checkbox" id ="'+columnDefs[j].name+'">';
                        }
            ...
            _addRowData: function () {
                ...
                $(`form[name="altEditor-add-form-${this.random_id}"] *`).filter(':input').each(function (i) {
					...
					if($(this)[0].outerHTML.indexOf("checkbox") >= 0){
						...
						if($(this)[0].id == "nom_canal"){
							if($(this).val() !=  ""){
								rowDataArray[$(this).attr('id')] = $(this).val();
							}else{
								//alert("Le nom du Canal est obligatoire");
								isError = true;
							}
						}else{
							rowDataArray[$(this).attr('id')] = $(this).val();
...
//console.log(rowDataArray); //DEBUG
				if(isError){
					that.onAddRowError(that,
						rowDataArray,
						function(data){ 
							that._addRowCallback(data); 
						},
						function(data){ that._errorCallbackInsert(data);
							}
					);
				}else{
					that.onAddRow(that,
            ...
            _deleteRowCallback: function (response, status, more) {
					...
					var url = 'http://localhost:8003/canal_d';
					...
					$.ajax(url,{
						...
						async: false,
						success: function (data_wrinserted,status,xhr) { 
            ...
            _addRowCallback: function (response, status, more) {
                    ...
                    var selector = this.modal_selector;
                    $(selector + ' .modal-body .alert').remove();
					data.fk_country = country;
					data.id_wr = -1;
					data.date_disponnible = currentDate;


					var url = 'http://localhost:8003/canal_p';
			...
			/**
             * Called after AJAX server returned an error Nom canal
             */
            _errorCallbackInsert: function (response, status, more) {
                    var error = response;
                    var selector = this.modal_selector;
                    $(selector + ' .modal-body .alert').remove();
                    var errstr = this.language.errorinsert.message;
                    var message = '<div class="alert alert-danger" role="alert">' +
                        '<strong>' + this.language.error.label + '</strong> ' + (error.status == null ? "" : this.language.error.responseCode + error.status) + " " + errstr +
                        '</div>';
			...
			/**
             * Default callback for Error OnAddRow.
             */
			
			onAddRowError: function(dt, rowdata, success, error) {
                error(rowdata);
            },
